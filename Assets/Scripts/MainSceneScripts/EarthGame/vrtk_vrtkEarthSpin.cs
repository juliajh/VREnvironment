...
namespace VRTK
{
    ...
    public class vrtk_vrtkEarthSpin : VRTK_BaseObjectControlAction
    {
        ...
        protected virtual void Move(GameObject controlledGameObject, Transform directionDevice, Vector3 axisDirection)
        {
            if (directionDevice && directionDevice.gameObject.activeInHierarchy && controlledGameObject.activeInHierarchy)
            {
                ...
                Compare(new Vector3(0, 0, 0), updatedPosition);
        ...
        void Compare(Vector3 a, Vector3 b)
        {
            if (a.x > b.x)
            {
                Earth.transform.Rotate(transform.up, 5f, Space.World);
            }
            else if (a.x < b.x)
            {
                Earth.transform.Rotate(transform.up, -5f, Space.World);
