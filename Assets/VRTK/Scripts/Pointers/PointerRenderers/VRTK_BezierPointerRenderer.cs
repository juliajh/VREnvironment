...
#pragma warning disable 0618
            ...
            downRayHit = VRTK_CustomRaycast.Raycast(customRaycast, projectedBeamDownRaycast, out collidedWith, layersToIgnore, maximumLength.y);
#pragma warning restore 0618
...
#pragma warning restore 0618
        ...
        {
            if (controllingPointer&& (destinationHit.transform.name=="Farm" || destinationHit.transform.parent.name== "waterPolluted"))
            {
                if (destinationHit.transform.name == "Farm")
                    infarm = true;
                else if(destinationHit.transform.parent.name == "waterPolluted")
                {
                    cleanpolluted = true;
        ...
        public bool getinfarm()
        {
            return infarm;
        }

        public bool getcleanpolluted()
        {
            return cleanpolluted;
        }

        public GameObject gettarget()
        {
            if (destinationHit.transform.gameObject!=null)
                return destinationHit.transform.gameObject;
            else
                return null;
        }
    }
